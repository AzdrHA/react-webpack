...
import { completeTask, deleteTask } from '@app/api/apiRequest';
import { changeTaskStatus, removeTask } from '@app/store/task';
...
const SingleTask: FC<ITaskProps> = ({ task }) => {
  ...
  const onCheckTask = async (e) => {
    setChecked(e.target.checked);
    completeTask(task.id, e.target.checked).then(() => {
      dispatch(changeTaskStatus({ id: task.id, completed: e.target.checked }))
    })
  }
    ...
    <div className="taskContainer" data-testid="list-item">
      <div className="taskContainer__content">
        <input
          ...
          checked={checked}
        />
        <label htmlFor={`todo-${task.id}`}>{task.title}</label>
      </div>
      <button onClick={onDeleteTaskAction} style={{ marginLeft: 'auto', marginRight: '1rem', cursor: 'pointer', background: 'none' }}>
